...
ENV MYSQL_PASSWORD Mysql123456
ENV DATABASE_NAME test_db
ENV APPLICATION_PORT 80

# 安装依赖包，如需其他依赖包，请到alpine依赖包管理(https://pkgs.alpinelinux.org/packages?name=php8*imagick*&branch=v3.13)查找。
RUN apk add --update --no-cache openjdk17-jre-base \
    && rm -f /var/cache/apk/*
...
# 将构建产物jar包拷贝到运行时目录中
COPY --from=build /app/target/test-0.0.1.jar .

# 暴露端口
EXPOSE 80

# 执行启动命令
CMD ["java", "-jar", "/app/test-0.0.1.jar", "--spring.profiles.active=test"]